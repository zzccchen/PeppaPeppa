...
from utility import add_arguments, print_arguments, to_lodtensor, get_ctc_feeder_data
...
# yapf: disable
# 训练时的参数在这里调整，包括批次大小、训练次数、保存模型的地址等等
add_arg('batch_size',        int,   32,         "Minibatch size.")
add_arg('total_step',        int,   720000,
...
add_arg('save_model_period', int,   1000,
        "Save model period. '-1' means never saving the model.")
add_arg('eval_period',       int,   1000,
...
def train(args):
    ...
    num_classes = None
    train_images = "data/ocr_recognition/data/train_images"
    train_list = "data/ocr_recognition/data/train.list"
    test_images = "data/ocr_recognition/data/test_images"
    test_list = "data/ocr_recognition/data/test.list"
    ...
    def train_one_batch(data):
        # 训练一个batch，该步骤用于观察训练完一个batch后模型的效果提升情况
